...
impl CookieJar {
    pub fn header_value_for_env(&self, env: &wombat_core::Env) -> String {
        self.cookies
            .iter()
            .filter(|c| c.env == *env)
...
pub struct CookieJarStatus {
    pub cookie_health: HashMap<wombat_core::Env, CookieHealth>,
}
...
impl CommandError {
    ...
    pub fn from_core(command: impl Into<String>, err: wombat_core::CoreError) -> CommandError {
        CommandError {
