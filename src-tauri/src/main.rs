...
struct NewTaskParams {
    port: u16,
    proxy_auth_config: Option<wombat_core::client::ProxyAuthConfig>,
}
...
struct ServiceDetailsPayload {
    app: String,
    services:
        Vec<Result<wombat_core::aws::ServiceDetails, wombat_core::aws::ServiceDetailsMissing>>,
    dbs: Vec<wombat_core::aws::RdsInstance>,
...
fn get_matching_services(
    ...
    services: Vec<wombat_core::aws::EcsService>,
) -> HashSet<String> {
...
async fn login(
    ...
    let environments: Vec<wombat_core::aws::Env>;
...
async fn credentials(
    app_handle: AppHandle,
    db: wombat_core::aws::RdsInstance,
    app_state: tauri::State<'_, AppContextState>,
    aws_config_provider: tauri::State<'_, AwsConfigProviderInstance>,
    rds_resolver_instance: tauri::State<'_, RdsResolverInstance>,
) -> Result<wombat_core::aws::DbSecret, CommandError> {
...
impl wombat_core::aws::LogSearchMonitor for WindowNotifier {
    fn notify(&mut self, logs: Vec<wombat_core::aws::LogEntry>) {
...
impl wombat_core::aws::LogSearchMonitor for FileNotifier {
    fn notify(&mut self, logs: Vec<wombat_core::aws::LogEntry>) {
...
async fn find_logs(
    ...
    let sdk_config: wombat_core::aws::SdkConfig;
    {
        let aws_config_provider = aws_config_provider.0.read().await;
    ...
    async_task_tracker.0.lock().await.search_log_handler = Some(tokio::task::spawn(async move {
        let _ = wombat_core::aws::find_logs(
            &sdk_config,
            env,
            apps,
            start_timestamp,
...
) -> Result<Vec<wombat_core::aws::Cluster>, CommandError> {
...
#[tauri::command]
async fn services(
    cluster: wombat_core::aws::Cluster,
    ecs_resolver_instance: tauri::State<'_, EcsResolverInstance>,
) -> Result<Vec<wombat_core::aws::EcsService>, CommandError> {
    let ecs_resolver_instance = ecs_resolver_instance.0.read().await;
    let services = ecs_resolver_instance.read_services().await;
    Ok(services
        .into_iter()
...
async fn deploy_ecs_service(
    ...
    let env = wombat_core::aws::Env::from_any(&cluster_arn);
...
async fn remove_task_definitions(
    service: wombat_core::aws::ServiceDetails,
    dry_run: bool,
    app_handle: AppHandle,
    app_state: tauri::State<'_, AppContextState>,
    aws_config_provider: tauri::State<'_, AwsConfigProviderInstance>,
    ...
    return Ok(wombat_core::aws::remove_non_platform_task_definitions(
        &aws_config,
        service.td_family,
        service.td_revision,
        dry_run,
...
async fn databases(
    env: wombat_core::aws::Env,
    rds_resolver_instance: tauri::State<'_, RdsResolverInstance>,
) -> Result<Vec<wombat_core::aws::RdsInstance>, CommandError> {
...
async fn discover(
    ...
    {
        let rds_resolver_instance = rds_resolver_instance.0.read().await;
        let found_dbs: Vec<wombat_core::aws::RdsInstance> = rds_resolver_instance
            .read_databases()
            .await
            .into_iter()
...
async fn service_details(
    ...
    let mut dbs_list: Vec<wombat_core::aws::RdsInstance> = Vec::new();
    {
    ...
    let services_to_resolve: Vec<wombat_core::aws::EcsService>;
    {
    ...
    let services = wombat_core::aws::service_details(
...
async fn start_db_proxy(
    ...
    let mut bastions = wombat_core::aws::bastions(&aws_config).await;
...
async fn start_service_proxy(
    app_handle: AppHandle,
    service: wombat_core::aws::EcsService,
    infra_profile: Option<wombat_core::aws::InfraProfile>,
    sso_profile: Option<wombat_core::aws::SsoProfile>,
...
async fn log_filters(
    wombat_api_instance: tauri::State<'_, WombatApiInstance>,
) -> Result<Vec<wombat_core::client::LogFilter>, CommandError> {
    let mut wombat_api = wombat_api_instance.0.lock().await;
...
async fn proxy_auth_configs(
    wombat_api_instance: tauri::State<'_, WombatApiInstance>,
) -> Result<Vec<wombat_core::client::ProxyAuthConfig>, CommandError> {
    let mut wombat_api = wombat_api_instance.0.lock().await;
...
async fn available_infra_profiles(
    aws_config_provider: tauri::State<'_, AwsConfigProviderInstance>,
) -> Result<Vec<wombat_core::aws::InfraProfile>, CommandError> {
...
async fn available_sso_profiles(
    aws_config_provider: tauri::State<'_, AwsConfigProviderInstance>,
) -> Result<Vec<wombat_core::aws::SsoProfile>, CommandError> {
    let provider = aws_config_provider.0.read().await;
    Ok(provider
        .active_wombat_profile
        .sso_profiles
...
async fn wombat_aws_profiles(
    aws_config_provider: tauri::State<'_, AwsConfigProviderInstance>,
) -> Result<Vec<wombat_core::aws::WombatAwsProfile>, CommandError> {
    let provider = aws_config_provider.0.read().await;
...
async fn open_dbeaver(
    ...
    fn db_beaver_con_parma(
        ...
        host: &str,
        port: u16,
        secret: &wombat_core::aws::DbSecret,
        read_only: bool,
    ) -> String {
    ...
    let secret;
    let found_db_secret = wombat_core::aws::db_secret(&aws_config, &db).await;
    match found_db_secret {
        Some(found_secret) => {
            secret = Ok(found_secret);
        }
...
async fn find_secret_with_fallback(
    config: &wombat_core::aws::SdkConfig,
    rds: &wombat_core::aws::RdsInstance,
    rds_resolver: Arc<RwLock<RdsResolver>>,
) -> Option<wombat_core::aws::DbSecret> {
    let found_secret = wombat_core::aws::db_secret(config, rds).await;
    if let Some(secret) = found_secret {
        return Some(secret);
    }

    let source_db = wombat_core::aws::find_source_rds(config, rds).await;
    let databases;
    {
...
async fn main() {
    ...
    let wombat_api = wombat_core::client::WombatApi::new(
    ...
    rest_api::serve(
        cookie_jar.clone(),
        browser_ext.clone(),
        Arc::new(Mutex::new(wombat_core::client::WombatApi::new(
            app_config.wombat_api_url.clone(),
            app_config.wombat_api_user.clone(),
            app_config.wombat_api_password.clone(),
            user.id,
    ...
    let aws_config_provider = Arc::new(RwLock::new(
        wombat_core::aws::AwsConfigProvider::new().await,
    ));
...
struct AppContext {
    active_profile: Option<wombat_core::aws::WombatAwsProfile>,
    aws_config_provider: Arc<RwLock<wombat_core::aws::AwsConfigProvider>>,
    last_auth_check: i64,
...
struct AwsConfigProviderInstance(Arc<RwLock<wombat_core::aws::AwsConfigProvider>>);
...
struct WombatApiInstance(Arc<Mutex<wombat_core::client::WombatApi>>);
...
async fn check_login_and_trigger(
    ...
    if !wombat_core::aws::is_logged(profile, config, fast_path).await {
        info!("Trigger log in into AWS");
        wombat_core::aws::cli_login(profile);
