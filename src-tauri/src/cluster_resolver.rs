...
impl ClusterResolver {
    ...
    pub async fn refresh(&mut self) -> Vec<wombat_core::aws::Cluster> {
    ...
    pub async fn clusters(&mut self) -> Vec<wombat_core::aws::Cluster> {
        ...
        for env in environments.iter() {
            ...
            let clusters = wombat_core::aws::clusters(&config).await;
    ...
    pub async fn read_clusters(&self) -> Vec<wombat_core::aws::Cluster> {
        info!("Resolving clusters");
        let pool = self.db_pool.clone();
        let clusters = tokio::task::block_in_place(|| {
            let conn = pool.get().unwrap();
...
fn fetch_clusters(conn: &Connection) -> Vec<wombat_core::aws::Cluster> {
    ...
    let rows = stmt.query_map([], |row| {
        ...
        let env: wombat_core::aws::Env =
            serde_json::from_str(&row.get::<usize, String>(2)?).unwrap();
        let platform_version: i32 = row.get(3).unwrap_or(0);
        Ok(wombat_core::aws::Cluster {
            arn,
            name,
            env,
            platform_version,
...
fn store_clusters(conn: &Connection, clusters: &[wombat_core::aws::Cluster]) {
    clear_clusters(conn);
