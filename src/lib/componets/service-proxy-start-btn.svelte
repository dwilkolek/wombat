<script lang="ts">
	...
	$: isAllowAllProxyEnabled = invoke<boolean>('is_feature_enabled', { feature: 'allow-all-proxies' });
</script>

{#await $availableProfilesStore then availableProfiles}
	{#await isAllowAllProxyEnabled then isAllowAllProxyEnabled}
		{#if isAllowAllProxyEnabled || availableProfiles.some((profile) => profile == service.name)}
					...
					>
						...
						{#await $availableProfilesStore then availableProfiles}
							{#each $proxyAuthConfigsStore as config}
								{@const disabled = !isAllowAllProxyEnabled && !availableProfiles.some(
