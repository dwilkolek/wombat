<script lang="ts">
	...
	$: isAllowAllSecretsEnabled = invoke<boolean>('is_feature_enabled', {
		feature: 'allow-all-secrets'
	});
</script>

{#if database}
	{#await $availableProfilesStore then availableProfiles}
		{#await isAllowAllSecretsEnabled then isAllowAllSecretsEnabled}
			{#if isAllowAllSecretsEnabled || availableProfiles.some((profile) => profile == database?.normalized_name)}
